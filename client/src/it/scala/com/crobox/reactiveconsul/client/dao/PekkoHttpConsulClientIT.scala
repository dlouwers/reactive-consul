...
import com.crobox.reactiveconsul.client.ClientITSpec
...
class PekkoHttpConsulClientIT extends ClientITSpec with RetryPolicy with Logging {
  val rnd                        = new Random()
  val subject                    = new PekkoHttpConsulClient(new URL(s"http://$host:$port"))
