...
import com.crobox.reactiveconsul.client.loadbalancers.{LoadBalancer, LoadBalancerActor, RoundRobinLoadBalancer}
import org.apache.pekko.actor.{ActorRef, ActorRefFactory}

case class ServiceDefinition(key: String,
                             serviceName: String,
                             serviceTags: Set[String] = Set.empty,
...
object ConnectionStrategy {
  ...
  def apply(serviceDefinition: ServiceDefinition,
            connectionProviderFactory: ConnectionProviderFactory,
            loadBalancer: LoadBalancer,
            onlyHealthyServices: Boolean): ConnectionStrategy =
    ConnectionStrategy(serviceDefinition,
                       connectionProviderFactory,
                       ctx => ctx.actorOf(LoadBalancerActor.props(loadBalancer, serviceDefinition.key)),
                       onlyHealthyServices)

  def apply(serviceDefinition: ServiceDefinition,
            connectionProviderFactory: (String, Int) => ConnectionProvider,
            loadBalancer: LoadBalancer,
            onlyHealthyServices: Boolean): ConnectionStrategy = {
  ...
  def apply(serviceName: String,
            connectionProviderFactory: (String, Int) => ConnectionProvider,
            loadBalancer: LoadBalancer): ConnectionStrategy =
    ConnectionStrategy(ServiceDefinition(serviceName),
                       connectionProviderFactory,
                       loadBalancer,
