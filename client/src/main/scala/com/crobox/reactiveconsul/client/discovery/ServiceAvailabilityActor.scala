...
import com.crobox.reactiveconsul.client.dao.{ConsulHttpClient, IndexedServiceInstances, ServiceInstance}
...
class ServiceAvailabilityActor(httpClient: ConsulHttpClient,
                               serviceDefinition: ServiceDefinition,
                               listener: ActorRef,
                               onlyHealthServices: Boolean)
    extends Actor {
  ...
  def updateServiceAvailability(
      services: IndexedServiceInstances
  ): (Option[ServiceAvailabilityUpdate], Future[IndexedServiceInstances]) = {
  ...
  def createServiceAvailabilityUpdate(oldState: IndexedServiceInstances,
                                      newState: IndexedServiceInstances): ServiceAvailabilityUpdate = {
...
object ServiceAvailabilityActor {
  ...
  def props(httpClient: ConsulHttpClient,
            serviceDefinition: ServiceDefinition,
            listener: ActorRef,
            onlyHealthyServices: Boolean): Props =
    Props(new ServiceAvailabilityActor(httpClient, serviceDefinition, listener, onlyHealthyServices))
  ...
  private[client] case class ServiceAvailabilityUpdate(key: String,
                                                       added: Set[ServiceInstance] = Set.empty,
